...
parser.add_argument('--plot', type=bool, default=False, help='plot the loss and acc and confusion matrix')
...
from PythonScripts.data_preparation import LoadData, H5pyToTensor, CreateDataLoader, CreateDataLoadersWithMultiDataset
...
from PythonScripts.visualization import plot_loss_curves, PlotConfusionMatrix
import matplotlib

# Set GPU first
device = SetDevice()

# Set the interaction model of matplotlib
if args.plot == False:
    matplotlib.use('Agg')
    print("Disable matplot lib")
...
train_labels = torch.squeeze(train_labels).long()             # [number_of_label, 1] --> [number_of_label]
validation_labels = torch.squeeze(validation_labels).long()
test_labels = torch.squeeze(test_labels).long()

# Create dataloader (from dataset list)
dataset_list = [train_data, validation_data]
labelset_list = [train_labels, validation_labels]
...
# Set up the learning rate schedulaer
lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 
                                                          mode='max', # set the min or max lr
                                                          patience=2, # how many epoch loss don't change
                                                          factor=0.1, # new_lr = old_lr * factor
                                                          threshold=0.1, # loss change
                                                          threshold_mode='rel', # compare mode
                                                          cooldown=2, # how many epoch to wait
                                                          min_lr=1e-8, # minimun of lr
...
if results["is_nan"] == []:
    ...
    PlotConfusionMatrix(y_true=y_true, 
                    y_pred=y_pred, 
